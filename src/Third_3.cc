...
main (int argc, char *argv[])
{
  ...
  app2->Setup (ns3TcpSocket02, sinkAddress2,3000,5000, DataRate ("1.5Mbps"));
  ...
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("Third_3_1.cwnd");
  ns3TcpSocket01->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&CwndChange, stream));
  
  Ptr<OutputStreamWrapper> stream1 = asciiTraceHelper.CreateFileStream ("Third_3_2.cwnd");
  ...
  Ptr<PcapFileWrapper> file1 = pcapHelper1.CreateFile ("sixth.pcap", std::ios::out, PcapHelper::DLT_PPP);
  devices1.Get (1)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop, file1));

  PcapHelper pcapHelper2;
  Ptr<PcapFileWrapper> file2 = pcapHelper2.CreateFile ("3rd2.pcap", std::ios::out, PcapHelper::DLT_PPP);
